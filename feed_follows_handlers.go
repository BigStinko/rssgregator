...
func (cfg *apiConfig) postFeedFollowsHandler(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	log.Printf(params.FeedID.String())
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Couldn't create feed follow")
		log.Print(err)
		return
...
func (cfg *apiConfig) deleteFeedFollowHandler(w http.ResponseWriter, r *http.Request, user database.User) {
	feedFolloIDStr := chi.URLParam(r, "feedFollowID")
	log.Printf("here %s", feedFolloIDStr)
	feedFollowID, err := uuid.Parse(feedFolloIDStr)
	if err != nil {
